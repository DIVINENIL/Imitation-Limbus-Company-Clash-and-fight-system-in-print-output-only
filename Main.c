...
SkillStats *getEffectiveSkill(Character *c, Character *c2, SkillStats *targetSkill, SkillStats *chosenSkill, int *tempOffense, int *tempDefense);
...
void applyDamage(Character *attacker, Character *defender, int damage, int trueDamage, const char *Type) {
  ...
  // Jia Qiu enemy heal
  if (attacker != NULL && isId(attacker->ID, "Jia Qiu") == 0 && (attacker->skills[15].active > 0) && defender->HP <= 0) {
      if (isId(defender->ID, "Hong lu:The Lord of Hongyuan") == 0) {
          attacker->skills[15].active -= 1;
          defender->HP = defender->MAX_HP;
          defender->FinalPowerUp[1] += 1;
          printf("\n%s's Uncompromising Imposition activated! Heal up to max HP and gain 1 Final Power Up next turn, lose 1 stack(%d)\n", defender->name, attacker->skills[15].active);
          sleep(1);
      } else {
          attacker->skills[15].active -= 1;
          defender->HP = defender->MAX_HP;
  ...
  if (isId(defender->ID, "Jia Qiu") == 0 && (defender->skills[15].active > 0) && attacker != NULL && attacker->HP <= 0) {
      if (isId(attacker->ID, "Hong lu:The Lord of Hongyuan") == 0) {
          defender->skills[15].active -= 1;
          attacker->HP = attacker->MAX_HP;
          attacker->FinalPowerUp[1] += 1;
          printf("\n%s's Uncompromising Imposition activated! Heal up to max HP and gain 1 Final Power Up next turn, lose 1 stack(%d)\n", attacker->name, defender->skills[15].active);
          sleep(1);
      } else {
          defender->skills[15].active -= 1;
          attacker->HP = attacker->MAX_HP;
...
void applySanityDebuff(Character *c) {
  ...
  if (c->Sanity <= -30 && c->Sanity > -45) {
    ...
    else if (isId(c->ID, "Shadow") == 0) {
      c->DefenseLevelDown[0] += 5;
      c->PlusCoinPowerBoost[0] += 2;
      printf("\nWhile 'Low Morale', gain 5 Defense Level Down, gain +2 Plus Coin Boost\n");
  ...
  if (c->Sanity <= -45) {
    if (isId(c->ID, "Lei heng") == 0) {
      ...
      sleep(1);
              ...
              c->DefenseLevelDown[0] += 10;
              c->FinalPowerUp[0] += 5;
              c->AttackPowerUp[0] += 5;
              c->DefensePowerUp[0] += 5;
              printf("\nWhile 'Shadō Gāden', gain 10 Defense Level Down, gain +5 Final Power Up, +5 Attack Power Up, +5 Defense Power Up\n");
...
void attackPhase(Character *attacker, SkillStats *atk, int atkTempOffense,
  ...
  int IsunableDefensetoact = isStaggered(defender);
  ...
  if (remainingCoins <= 0) {
    ...
    if (defSkill != NULL && defSkill->skillType == 4 && !IsunableDefensetoact && !ClashLostAttack && clashCount > 0 && defSkill->active == 1 && !nodefense) {
      
      powerReduction = defSkill->BasePower + defSkill->BasePowerBoost[0];
  ...
  //--------------------------- Before Attack Buff ----------------------------
    ...
    if (isId(defender->ID, "Lei heng") == 0 && defender->skills[6].active == 1 && atk->skillType == 0) {
      ...
      else if (isId(attacker->ID, "Lei heng") == 0 && attacker->skills[6].active == 1 && defSkill != NULL && defSkill->skillType != 0) {
    ...
    if (isId(attacker->ID, "Lei heng") == 0 && clashCount <= 0 && (defSkill == NULL || defSkill->skillType == 0)) {
  ...
  if (isId(attacker->ID, "Dawn Office Fixer Sinclair") == 0 && !attacker->skills[3].active && attacker->Sanity >= 45) {
    ...
    if (isId(attacker->ID, "Dawn Office Fixer Sinclair") == 0 && attacker->skills[3].active && attacker->Sanity >= 45 && !attacker->skills[4].active) {
  ...
  if (isId(attacker->ID, "Dawn Office Fixer Sinclair") == 0 && (atk == &attacker->skills[3] || atk == &attacker->skills[2]) && attacker->skills[3].active) {
      ...
      int boost = (defender->Burn[0]) * 4;
    if (boost > 120) boost = 120;

      atk->DamageUp[0] += boost;

      printf("\n%s gains +4%% Damage (%d%% - Max 120%%) for every 1 Burn Stack on target (%d)\n",
             attacker->name, boost, defender->Burn[0]);

      sleep(1);

    if (attacker->Sanity <= 40) {

    boost = (attacker->Sanity) * 10;
    if (boost > 60) boost = 60;

      atk->DamageUp[0] += boost;

      printf("\n%s at less than 40 Sanity, gains +10%% Damage (%d%% - Max 60%%) for every 5 Sanity (%d)\n",
             attacker->name, boost, attacker->Sanity);

      sleep(1);

    } else if (attacker->Sanity >= 40) {

      boost = (attacker->Sanity) * 15;
      if (boost > 120) boost = 120;

        atk->DamageUp[0] += boost;

        printf("\n%s at 40+ Sanity, gains +15%% Damage (%d%% - Max 120%%) for every 5 Sanity (%d)\n",
               attacker->name, boost, attacker->Sanity);
...
if (isId(attacker->ID, "Gregor:Firefist") == 0 && (atk == &attacker->skills[2])) {
  ...
  if (isId(attacker->ID, "Hong lu:The Lord of Hongyuan") == 0 &&
    ...
    if (atk == &attacker->skills[3]) buff = 30;
  ...
  //----------------------------------------------------------------
    ...
    if (isId(defender->ID, "The House of Spiders: The Thumb Nursefather Rodion") == 0 && atk->skillType != 3 && defender->Passive > 0 && defender->skills[11].active == 0 && Evaded == 0 && !isStaggered(defender) && !isPanicked(defender)) {
        Evaded = 1;
      IsStillEvaded = 1;
  ...
  for (int i = 0; i < remainingCoins; i++) {
    ...
    float damageModifier = 0;
      ...
      if (defSkill != NULL && defSkill->skillType == 4 && powerReduction > 0) {
        modifiedPower = (currentPower - powerReduction) + (((currentPower - powerReduction) * damageModifier) / 100.0f);
      }
...
if (modifiedPower < 0.0f) modifiedPower = 0.0f;
    ...
    //-----------------------------------------
      ...
      // Skill 1
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[0]) {

        if (i == remainingCoins - 5) {
          printf("\n\n%s: \"There is no use to run now.\"\n", attacker->name);
      ...
      // Skill 3
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[2]) {

        if (i == remainingCoins - 3) {
          printf("\n\n%s: \"This areas are under my control\"\n", attacker->name);

          sleep(1);
        }    

        if (i == remainingCoins - 1) {
          printf("\n\n%s: \"Let that truth be carved into your soul!\"\n", attacker->name);
      ...
      // Skill 4
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[3]) {

        if (i == 0) {
          printf("\n\n%s: \"Allow me to show you the witch's secret art.\"\n", attacker->name);
      ...
      // Skill 5
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[4]) {

        if (i == 0) {
          printf("\n\n%s: \"Let this flame burn you to ash.\"\n", attacker->name);
      ...
      // Skill 6
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[5]) {
      ...
      // Skill 7
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[6]) {

        if (i == 0) {
          printf("\n\n%s: \"You messed with me and my subordinate?\"\n", attacker->name);
      ...
      // Skill 8
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[7]) {
        ...
        if (i == 0) {
          ...
          printf("\n%s: \"And witness for yourselves!\"\n", attacker->name);

          sleep(1);

          printf("\n%s: \"My earth-shattering...\"\n", attacker->name);
          ...
          printf("\n%s: \"supremely ultimate, almighty and unparalleled attack!\"\n", attacker->name);
    ...
    if (isId(attacker->ID, "Hong lu:The Lord of Hongyuan") == 0 && (atk == &attacker->skills[1] || atk == &attacker->skills[4]) && i == remainingCoins - 1 && IsCritical) {
      ...
      // Hong lu:The Lord of Hongyuan S4 Last coins
      if (isId(attacker->ID, "Hong lu:The Lord of Hongyuan") == 0 && (atk == &attacker->skills[3]) && i == remainingCoins - 1) {

        int gain = 15 * attacker->Passive;
        if (gain > 50) gain = 50;

          atk->DamageUp[0] += gain;

           printf("\n%s's last coin deal +15%% damage for every Heishou Bolus Contamination [黑獸丸染] on self (%d%% - Max 50%%)", attacker->name, gain);
    ...
    if (isId(attacker->ID, "Meursault:Blade Lineage Mentor") == 0 && (atk == &attacker->skills[1]) && i == 2 && IsCritical) {
      ...
      if (i == remainingCoins - 1 && isId(attacker->ID, "Meursault:Blade Lineage Mentor") == 0 && atk == &attacker->defenseSkill[1] && attacker->defenseSkill[2].active == 1) {
          float finalCoinBonus = (float)((attacker->Poise[0] + attacker->Poise[1])) / 100.0f;
    ...
    if (isId(attacker->ID, "Fixer grade 9?") == 0 && (atk == &attacker->skills[7]) && i == remainingCoins - 1) {
      ...
      // Shadow Skill 2 Last coins
      if (isId(attacker->ID, "Shadow") == 0 && (atk == &attacker->skills[1]) && i == remainingCoins - 1 && attacker->Passive >= 200) {

          atk->DamageUp[0] += 50;

           printf("\n%s's last coin consumes 200 Mana (%d) to deal +50%% damage", attacker->name, attacker->Passive);

      }

      // Shadow Skill 7 Last coins
      if (isId(attacker->ID, "Shadow") == 0 && (atk == &attacker->skills[6]) && i == remainingCoins - 1 && attacker->Passive >= 100) {

          atk->DamageUp[0] += 30;

           printf("\n%s's last coin consumes 100 Mana (%d) to deal +30%% damage", attacker->name, attacker->Passive);
...
// --------------------------------------------------------------------------------------
      ...
      // Shadow - Violet Flame Enchant
      if (isId(attacker->ID, "Shadow") == 0 && attacker->skills[1].active > 0) {
        int damagebonus = 20;
        if (attacker->Burn[0] > 0) damagebonus = 50;

        Damage *= 1 + (damagebonus/100);
      }

      // Shadow Skill 7 coins
      if (isId(attacker->ID, "Shadow") == 0 && (atk == &attacker->skills[6]) && IsCritical) {

          Damage *= 1.15;

           printf("\n%s's this coin deal +15%% damage on Critical Hit", attacker->name);
...
// The Index Nursefather Yi Sang: Oracle Device [Caduceus] Weapon Name + Other
  ...
  // ---------------- Keyword Status ----------------
    ...
    printf("\t Critical Hit! (Chance %d%% Stack %d Count %d)", PoiseChance, attacker->Poise[0], attacker->Poise[1]);
      ...
      if (defender->Rupture[0] <= 0 && defender->Rupture[1] > 0) defender->Rupture[0]++;
          ...
          int deal = defender->Rupture[0];
           ...
           if (defender->Rupture[1] <= 0) defender->Rupture[1] = 0;
            ...
            printf("\t Deal %d Rupture damage on enemy (Stack %d Count %d)", deal, defender->Rupture[0], defender->Rupture[1]);
    ...
    if ((attacker->Bleed[0] > 0 || attacker->Bleed[1] > 0) && atk->skillType == 0) {
      ...
      printf("\t Take %d Bleed damage (Stack %d Count %d)", damage, attacker->Bleed[0], attacker->Bleed[1]);
      ...
      if ((defender->Sinking[0] > 0 || defender->Sinking[1] > 0) && !Evaded) {
        ...
        if (defender->Sinking[0] <= 0 && defender->Sinking[1] > 0) defender->Sinking[0]++;
          ...
          printf("\t Sanity -%d on enemy (%d) (Stack %d Count %d)", deal, defender->Sanity, defender->Sinking[0], defender->Sinking[1]);
            ...
            defender->Sinking[1] -= 1;
             ...
             if (defender->Sinking[1] <= 0) defender->Sinking[1] = 0;
              ...
              printf("\t Deal %d Sinking damage on enemy (Stack %d Count %d)", deal, defender->Sinking[0], defender->Sinking[1]);
  ...
  // ----------------------- Inflict Status -----------------------
      ...
      // Passive
      if (isId(attacker->ID, "Shadow") == 0 && !Evaded) {
          // Slime Weapon: On Hit deal 5% bonus damage
          int slimeBonus = (int)(finalDamage * 0.05);
          if (slimeBonus < 1 && finalDamage > 0) slimeBonus = 1;

         // Slime Weapon: Uncreaking Final Coinโบนัส 20% (เช็คว่าเป็นเหรียญสุดท้ายและไม่แตก)
          if (i == remainingCoins - 1 && Unbreakable <= 0) {
              int finalBonus = (int)(finalDamage * 0.20);
              applyDamage(attacker, defender, finalBonus, 0, NULL);
              totalDamage += finalBonus;
              printf("\t [On Hit without Creaking] Deal %d bonus damage", finalBonus);
          }

          applyDamage(attacker, defender, slimeBonus, 0, NULL);
          totalDamage += slimeBonus;
        printf("\t [On Hit] Deal %d bonus damage", slimeBonus);

          // Slime Weapon: On Hit inflict 1 Bleed
          inflictStatus(defender->Bleed, 1, 0, 0, 99, 0, 99);
        printf("\t [On Hit] Bleed Stack +1 on enemy (%d)", defender->Bleed[0]);

          // If Enchanted (skills[1].active > 0), inflict 1 Violet Burn
          if (attacker->skills[1].active > 0) {
              // ใช้ระบบ Burn เดิม แต่ Shadow จะทำให้มันกลายเป็น Violet Burn ในช่วง Turn End
              inflictStatus(defender->Burn, 1, 1, 0, 99, 0, 99);
              printf("\t [On Hit] Violet Burn Stack +1 (%d) and Violet Burn Count +1 (%d) on enemy", defender->Burn[0], defender->Burn[1]);
          }

      }

      // Shadow - Skill 1
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[0] && !Evaded) {
         
          if (i == remainingCoins - 5 || i == remainingCoins - 4 || i == remainingCoins - 3) {
            inflictStatus(defender->Bleed, 1, 0, 0, 99, 0, 99);
            printf("\t [On Hit] Bleed Stack +1 on enemy (%d)", defender->Bleed[0]);
          }
      ...
      // Shadow - Skill 2
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[1] && !Evaded) {

        if (i == remainingCoins - 2) {
          inflictStatus(defender->Tremor, 10, 0, 0, 99, 0, 99);
          printf("\t [On Hit] Tremor Stack +10 on enemy (%d)", defender->Tremor[0]);
        }

        if (i == remainingCoins - 1) {
          printf("\t [On Hit]");
          TremorBurst(attacker, defender, 100, &totalDamage, 1);
        }

      }

      // Shadow - Skill 3
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[2] && !Evaded) {
        ...
        if (i == remainingCoins - 1) {
          defender->Bind[1] += 3;
            printf("\t [On Hit] Bind +3 on enemy next turn");
        }

      }

      // Shadow - Skill 4
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[3] && !Evaded) {

        if (i == remainingCoins - 4 || i == remainingCoins - 6) {
          inflictStatus(defender->Tremor, 3, 0, 0, 99, 0, 99);
          printf("\t [On Hit] Tremor Stack +3 on enemy (%d)", defender->Tremor[0]);
          printf("\t [On Hit] Raise Stagger Threshold by %d", 10);
          MoveStagger(attacker, defender, 100, 10, 1);
        }

        if (i == remainingCoins - 3 || i == remainingCoins - 5) {
        inflictStatus(defender->Tremor, 0, 2, 0, 99, 0, 99);
        printf("\t [On Hit] Tremor Count +2 on enemy (%d)", defender->Tremor[1]);
          printf("\t [On Hit] Raise Stagger Threshold by %d", 10);
          MoveStagger(attacker, defender, 100, 10, 1);
        }
      ...
      // Shadow - Skill 5
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[4] && !Evaded) {

        if (i == remainingCoins - 1) {
          inflictStatus(defender->Burn, 5, 3, 0, 99, 0, 99);
          printf("\t [On Hit] Violet Burn Stack +5 (%d) and Violet Burn Count +3 (%d) on enemy", defender->Burn[0], defender->Burn[1]);
          attacker->skills[1].active += 3;
          if (attacker->skills[1].active > 5) attacker->skills[1].active = 5;
          printf("\t [On Hit] Violet Flame Enchant +3 on self (%d)", attacker->skills[1].active);
        }

      }

      // Shadow - Skill 6
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[5] && !Evaded) {
        ...
        if (i == remainingCoins - 1) {
          int damage = defender->Bleed[0] > 0 ? defender->Bleed[0] : 1;

              defender->Bleed[1]--;

          if (defender->Bleed[1] <= 0) defender->Bleed[1] = 0;

          printf("\t [On Hit] Activate Bleed on target (%d). Target loses 1 Bleed Count (%d)", defender->Bleed[0], defender->Bleed[1]);

          if (defender->Bleed[1] <= 0) defender->Bleed[0] = 0;

          applyDamage(NULL, defender, damage, 0, "Bleed");
          totalDamage += damage;
        }
      ...
      // Shadow - Skill 7
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[6] && !Evaded) {

        if (i == remainingCoins - 4 || i == remainingCoins - 5) {
          inflictStatus(defender->Bleed, 3, 0, 0, 99, 0, 99);
          printf("\t [On Hit] Bleed Stack +3 on enemy (%d)", defender->Bleed[0]);
        }

        if (i == remainingCoins - 2 || i == remainingCoins - 3) {
        inflictStatus(defender->Bleed, 0, 3, 0, 99, 0, 99);
        printf("\t [On Hit] Bleed Count +3 on enemy (%d)", defender->Bleed[1]);
        }
      ...
      // Shadow - Skill 8
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[7] && !Evaded) {

        if (i == remainingCoins - 1) {

          if (Unbreakable <= 0) {
          int finalBonus = defender->Burn[0] + defender->Bleed[0] + defender->Tremor[0];
          applyDamage(attacker, defender, finalBonus, 0, NULL);
          totalDamage += finalBonus;
          printf("\t [On Hit without Creaking] Deal (Burn Stack on target + Bleed Stack on target + Tremor Stack on target) bonus damage (%d)", finalBonus);
          }

          inflictStatus(defender->Burn, 10, 5, 0, 99, 0, 99);
          printf("\t [On Hit] Violet Burn Stack +10 (%d) and Violet Burn Count +5 (%d) on enemy", defender->Burn[0], defender->Burn[1]);

          int damage = (defender->Burn[0] > 0 ? defender->Burn[0] : 1) * 2;

              defender->Burn[1]--;

          if (defender->Burn[1] <= 0) defender->Burn[1] = 0;

          printf("\t [On Hit] Activate Burn on target (Damage %d Stack %d). Target loses 1 Burn Count (%d)", damage, defender->Burn[0], defender->Burn[1]);

          if (defender->Burn[1] <= 0) defender->Burn[0] = 0;

          applyDamage(NULL, defender, damage, 0, "Burn");
          totalDamage += damage;
      ...
      // Shadow - Skill 9
      if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[8] && !Evaded) {

        if (i == remainingCoins - 3) {
          inflictStatus(defender->Burn, 2, 1, 0, 99, 0, 99);
          printf("\t [On Hit] Violet Burn Stack +2 (%d) and Violet Burn Count +1 (%d) on enemy", defender->Burn[0], defender->Burn[1]);
        }
        ...
        if (i == remainingCoins - 2) {
          ...
          printf("\t [On Hit] Bind +1 on enemy next turn");
          defender->Haste[1] += 2;
          printf("\t [On Hit] Haste +2 on self next turn");
      ...
      //Dawn Office Fixer Sinclair Defense
      if (isId(defender->ID, "Dawn Office Fixer Sinclair") ==
        0 && defSkill == &defender->defenseSkill[0] && !Evaded) {

        int Stack = 1;

        if (defender->defenseSkill[1].active >= 30) {Stack > 0 ? Stack++ : 0;}

        if (Stack > 0) {
          inflictStatus(attacker->Burn, Stack, 0, 0, 99, 0, 99);
          printf("\t [On Guard] Burn Stack +%d on enemy (%d)", Stack, attacker->Burn[0]);
        }

          defender->defenseSkill[1].active += Stack;

        }

      //Dawn Office Fixer Sinclair S1
      if (isId(attacker->ID, "Dawn Office Fixer Sinclair") ==
        0 && atk == &attacker->skills[0] && !Evaded) {
        ...
        if (attacker->defenseSkill[1].active >= 30) {Stack > 0 ? Stack++ : 0; Count > 0 ? Count++ : 0;}
        if (attacker->skills[3].active) {Stack++;}
        ...
        attacker->defenseSkill[1].active += Stack;

        }

      //Dawn Office Fixer Sinclair S2
      if (isId(attacker->ID, "Dawn Office Fixer Sinclair") ==
        0 && atk == &attacker->skills[1] && !Evaded) {
      ...
      //Dawn Office Fixer Sinclair S3-1
      if (isId(attacker->ID, "Dawn Office Fixer Sinclair") ==
        0 && !attacker->skills[3].active && (atk == &attacker->skills[2] || atk == &attacker->skills[3]) && !Evaded) {
      ...
      //Dawn Office Fixer Sinclair S3-2
      if (isId(attacker->ID, "Dawn Office Fixer Sinclair") ==
        0 && attacker->skills[3].active && (atk == &attacker->skills[2] || atk == &attacker->skills[3]) && !Evaded) {

        if (defender->Burn[0] >= 10) {

          attacker->defenseSkill[1].active += 2;
          
          inflictStatus(defender->Burn, 2, 0, 0, 99, 0, 99);
          printf("\t [On Hit] Target has 10+ Burn Stack, Burn Stack +2 (%d) and Burn Count +1 (%d) on enemy", defender->Burn[0], defender->Burn[1]);
        ...
        if (IsHeadHit) {
          ...
          if (Stack > 0) {
            inflictStatus(defender->Burn, Stack, 0, 0, 99, 0, 99);
            printf("\t [Heads Hit] Burn Stack +%d on enemy (%d)", Stack, defender->Burn[0]);
          }

          if (Count > 0) {
            inflictStatus(defender->Burn, 0, Count, 0, 99, 0, 99);
            printf("\t [Heads Hit] Burn Count +%d on enemy (%d)", Count, defender->Burn[1]);
          }
      ...
      // Sukuna - Black Flash
      if (isId(attacker->ID, "Sukuna:King of Curse") == 0 &&
          atk == &attacker->skills[5] && !Evaded && IsHeadHit) {
        ...
        printf("\t [Heads Hit] Trigger 'Black Flash', Deal 2.5x more damage , HP +%d (%.2f), Sanity +10 (%d)", heal, attacker->HP, attacker->Sanity);
      ...
      // --------------- Lei heng -----------------
            ...
            if (isId(attacker->ID, "Lei heng") == 0 && !Evaded) {
              ...
              if (defSkill != NULL && defSkill->skillType == 0) {
              // Unopposed Attacks
              if (clashCount <= 0) {
              ...
              if (SpendAmmoLeiheng > 0 || SpendSavageAmmoLeiheng > 0) {
                ...
                printf("\t [On Hit] Defense Level Down +1 on enemy next turn");
              }
              }
...
// --------------------------------------------------------------------
    ...
    if (isId(attacker->ID, "Hong lu:The Lord of Hongyuan") == 0 && atk == &attacker->skills[2] && !Evaded) {
      ...
      if (i == remainingCoins - 1) {
        inflictStatus(defender->Rupture, 0, 4, 0, 99, 0, 99);
        printf("\t [On Hit] Rupture Count +4 on enemy (%d)", defender->Rupture[1]);
      }
...
// --------------------------------------------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
    ...
    if (isId(attacker->ID, "Dawn Office Fixer Sinclair") ==
      0 && attacker->skills[3].active && i == 0 && atk == &attacker->skills[0] && !Evaded && IsHeadHit) {
      ...
      if (Unbreakable > 0) {
       ...
       printf("\n[Heads Hit] %s in a Volatile E.G.O State, Reuse Coin (once per Skill)", attacker->name);
      ...
      //Shadow S5 REUSE
      if (isId(attacker->ID, "Shadow") ==
        0 && attacker->skills[1].active > 0 && i == 0 && atk == &attacker->skills[4] && !Evaded) {
        ...
        Unbreakable++;
         ...
         printf("\n[On Hit] %s has 'Violet Flame Enchant', Reuse Coin (once per Skill)", attacker->name);
  ...
  //---------------- After Attack Buff ----------------------------
    ...
    if (isId(attacker->ID, "Shadow") == 0 && atk == &attacker->skills[7]) {
       ...
       attacker->skills[0].active = 0; // Reset True Power

      attacker->Haste[1] += 5;

    printf("\n%s consumed all 'True Power' and gains +5 Haste next turn\n", attacker->name);
    ...
    if (isId(defender->ID, "The House of Spiders: The Thumb Nursefather Rodion") == 0 && defender->skills[4].active > 0 && defSkill->Unbreakable <= 0) {
...
// ------------------------ The House of Spiders: The Index Nursefather Yi Sang ---------------------------
  ...
  // -----------------------------------------------------------------------------------------
   ...
   // Hong lu:The Lord of Hongyuan S3-1 Debuff
    ...
    if (isId(attacker->ID, "Dawn Office Fixer Sinclair") == 0 && attacker->skills[3].active && (atk == &attacker->skills[1])) {

        updateSanity(attacker, -(5));
        if (attacker->Sanity < -45) attacker->Sanity = -45;

        printf("\n%s loses 5 Sanity (%d Sanity)\n",
               attacker->name, attacker->Sanity);
  ...
  if (isId(attacker->ID, "Lei heng") == 0 &&
    ...
    attacker->ClashPowerDown[1] += clashpowerdebuff;
      ...
      attacker->skills[9].active = 0; // reset ose clash power progess
  ...
  if (isId(attacker->ID, "Hong lu:The Lord of Hongyuan") == 0 &&
    ...
    defender->OffenseLevelDown[1] += 5;

    printf("\n%s gains 5 Offense Down next turn by %s's Skill\n", defender->name, attacker->name);

    } else if (!randomlost) {

        defender->DefenseLevelDown[1] += 5;

        printf("\n%s gains 5 Defense Down next turn by %s's Skill\n", defender->name, attacker->name);
...
SkillStats *getEffectiveSkill(Character *c, Character *c2, SkillStats *targetSkill,
  ...
  *tempDefense += chosenSkill->Defense;
  ...
  // Passive at high Speed
  if (isId(c->ID, "Shadow") == 0) {
    // เงื่อนไข: Speed 15+ หรือ Haste 5+ 
    // และต้องเช็คว่าเทิร์นนี้ยังไม่ได้ทำงาน (ใช้ skills[3].active เป็นตัวเก็บ TurnCount ล่าสุดที่ทำงาน)
  if ((c->Speed >= 15 || c->Haste[0] >= 5) && c->skills[3].active != TurnCount) {

      c->skills[3].active = TurnCount; // ปักป้ายบอกว่า "เทิร์นนี้ตีฟรีไปแล้วนะ"

      // 1. Gain 2 Violet Flame Enchant (ตัวแปร skills[1].active)
      c->skills[1].active += 2;
      if (c->skills[1].active > 5) c->skills[1].active = 5;

    printf("\n%s gains 2 Violet Flame Enchant (%d)\n", c->name, c->skills[1].active);
    sleep(1);

      printf("\n%s: \"There is no road to almightiness for those using borrowed strength.\"\n", c->name);
      sleep(1);

    printf("\n%s: \"NONE!\"\n", c->name);
    sleep(1);

      // 2. รัน Unopposed Attack ด้วย Skill 8
      // คำนวณค่าพลังโจมตี/ป้องกัน ณ ขณะนั้น
      int currentAtkOff = *tempOffense + c->skills[8].Offense;
      int currentAtkDef = *tempDefense + c->skills[8].Defense;

      // ดึงค่าฝั่งศัตรูมาอ้างอิง (c2 คือเป้าหมาย, targetSkill คือสกิลที่ศัตรูกำลังจะใช้)
      int targetOff = (c2->OffenseLevelUp[0] - c2->OffenseLevelDown[0]) + targetSkill->Offense;
      int targetDef = (c2->DefenseLevelUp[0] - c2->DefenseLevelDown[0]) + targetSkill->Defense;

      // สั่งโจมตีฟรีทันที
      attackPhase(c, &c->skills[8], currentAtkOff, currentAtkDef, 
                  c2, targetSkill, targetOff, targetDef, 
                  c->skills[8].Coins, 0, 0);
  ...
  // Passive
  if (isId(c->ID, "Shadow") == 0) {
      // Gain 1 Offense/Defense Level per True Power Stack
      int tp = c->skills[0].active;
      *tempOffense += tp;
      *tempDefense += tp;

    printf("\n%s gains %d Offense Level Up and %d Defense Level Up from True Power (%d)\n", c->name, tp, tp, c->skills[0].active);

    sleep(1);

    updateSanity(c, tp);
    printf("\n%s heals Sanity equal to 'True Power' (%d) (Sanity %d)\n", c->name, tp, c->Sanity);

    sleep(1);

      // Mana (Passive) - ทุกๆ 100 Mana เพิ่มเกราะ 10% (ใช้ Protection)
      int mana = c->Passive;
      float prot = (mana / 100) * 10.0f;
      if (prot > 50.0f) prot = 50.0f;
      chosenSkill->Protection[0] += prot;

    printf("\n%s takes -%.0f%% damage (Max 50%%)\n", c->name, prot);

    sleep(1);
  }

  // Skill Defense
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->defenseSkill[0])) {
    ...
    c->Passive += 200;
    if (c->Passive > 1000) c->Passive = 1000;
    printf("\n%s gains +200 Mana (%d)\n", c->name, c->Passive);

     sleep(1);

     int gain = c2->Bleed[0] + c2->Burn[0]/4;
     if (gain > 3) gain = 3;
     ...
     if (gain > 0) {
      ...
      printf("\n%s gains +1 Final Power (%d - Max 3) for every 4 (Bleed Stack(%d) + Unique Burn Stack(%d)) on target (%d)\n", c->name, gain, c2->Bleed[0], c2->Burn[0], c2->Bleed[0] + c2->Burn[0]);
  ...
  // Skill 1
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->skills[0])) {

    if (c->Passive >= 100) {
    c->Passive -= 100;
    if (c->Passive < 0) c->Passive = 0;
    printf("\n%s consumes 100 Mana (%d) to gains +2 Final Power\n", c->name, c->Passive);
    ...
    chosenSkill->FinalPowerBoost[0] += 2;
     ...
     int gain = c2->Bleed[0] + c2->Burn[0]/6;
     if (gain > 1) gain = 1;
     ...
     if (gain > 0) {
      ...
      printf("\n%s at 6 (Bleed Stack(%d) + Unique Burn Stack(%d)) (%d), %s gains +1 Coin Power\n", c2->name, c2->Bleed[0], c2->Burn[0], c2->Bleed[0] + c2->Burn[0], c->name);
    ...
    gain = c->Poise[0]/5;
     ...
     if (gain > 0) {
      ...
      printf("\n%s at 5+ Poise Stack (%d), %s gains +1 Final Power\n", c2->name, c2->Poise[0], c->name);

       sleep(1);

     }

    inflictStatus(c->Poise, 0, 5, 0, 99, 0, 99);
    printf("\n%s gains +5 Poise Count (%d)\n", c->name, c->Poise[1]);

     sleep(1);

    c->Haste[1] += 1;
    printf("\n%s gains +1 Haste next turn\n", c->name);

     sleep(1);
  }

  // Skill 2
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->skills[1])) {
    
     int gain = c2->Bleed[0] + c2->Burn[0]/4;
     if (gain > 2) gain = 2;

     if (gain > 0 && c->skills[1].active > 0) {

       chosenSkill->FinalPowerBoost[0] += gain;

      printf("\n%s has 'Violet Flame Enchant', gains +1 Final Power (%d - Max 2) for every 4 (Bleed Stack(%d) + Unique Burn Stack(%d)) on target (%d)\n", c->name, gain, c2->Bleed[0], c2->Burn[0], c2->Bleed[0] + c2->Burn[0]);
    ...
    gain = c2->Tremor[0]/5;
     ...
     if (gain > 0) {
      ...
      printf("\n%s at 5+ Tremor Stack (%d), %s gains +2 Coin Power\n", c2->name, c2->Tremor[0], c->name);

       sleep(1);

     }

    inflictStatus(c->Poise, 3, 0, 0, 99, 0, 99);
  ...
  // Skill 3
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->skills[2])) {

    if (c->Passive >= 150) {
    c->Passive -= 150;
    if (c->Passive < 0) c->Passive = 0;
    printf("\n%s consumes 200 Mana (%d) to gains +2 Final Power\n", c->name, c->Passive);
    ...
    chosenSkill->FinalPowerBoost[0] += 2;
     ...
     if (gain > 0 && c->skills[1].active > 0) {
      ...
      printf("\n%s has 'Violet Flame Enchant', gains +1 Final Power (%d - Max 3) for every 4 (Bleed Stack(%d) + Unique Burn Stack(%d)) on target (%d)\n", c->name, gain, c2->Bleed[0], c2->Burn[0], c2->Bleed[0] + c2->Burn[0]);
    ...
    gain = c->Poise[0]/5;
     ...
     if (gain > 0) {
      ...
      printf("\n%s at 5+ Poise Stack (%d), %s gains +1 Coin Power\n", c2->name, c2->Poise[0], c->name);

       sleep(1);

     }

    inflictStatus(c->Poise, 3, 3, 0, 99, 0, 99);
    printf("\n%s gains +3 Poise Stack (%d) and +3 Poise Count (%d)\n", c->name, c->Poise[0], c->Poise[1]);

     sleep(1);
  }

  // Skill 4
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->skills[3])) {

    if (c->Passive >= 200) {
    c->Passive -= 200;
    if (c->Passive < 0) c->Passive = 0;
    printf("\n%s consumes 200 Mana (%d) to gains +1 Base Power\n", c->name, c->Passive);
    ...
    if (gain > 3) gain = 3;
     ...
     if (gain > 0) {
      ...
      printf("\n%s gains +1 Final Power (%d - Max 3) for every 4 Tremor on target (%d)\n", c->name, gain, c2->Tremor[0]);
  ...
  // Skill 5
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->skills[4])) {

    if (c->Passive >= 200) {
    c->Passive -= 200;
    if (c->Passive < 0) c->Passive = 0;
    printf("\n%s consumes 200 Mana (%d) to convert to Unbreakable Coin\n", c->name, c->Passive);

    chosenSkill->UnbreakableUp[0] += 1;
    }
     ...
     if (gain > 0) {
      ...
      printf("\n%s gains +1 Coin Power (%d - Max 5) for every 4 (Bleed Stack(%d) + Unique Burn Stack(%d)) on target (%d)\n", c->name, gain, c2->Bleed[0], c2->Burn[0], c2->Bleed[0] + c2->Burn[0]);
    ...
    if (gain > 3) gain = 3;
     ...
     if (gain > 0) {
      ...
      printf("\n%s gains +1 Final Power (%d - Max 3) for every 4 Poise on self (%d)\n", c->name, gain, c2->Poise[0]);
    ...
    c->skills[1].active += 3;
    if (c->skills[1].active > 5) c->skills[1].active = 5;
    printf("\n%s gains +3 Violet Flame Enchant (%d)\n", c->name, c->skills[1].active);

     sleep(1);

    inflictStatus(c->Poise, 0, 3, 0, 99, 0, 99);
    printf("\n%s gains +3 Poise Count (%d)\n", c->name, c->Poise[1]);

     sleep(1);
  }

  // Skill 6
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->skills[5])) {

    if (c->Passive >= 200) {
    c->Passive -= 200;
    if (c->Passive < 0) c->Passive = 0;
    printf("\n%s consumes 200 Mana (%d) to gains +2 Haste next turn\n", c->name, c->Passive);
    ...
    if (gain > 2) gain = 2;
     ...
     if (gain > 0) {
      ...
      printf("\n%s gains +1 Coin Power (%d - Max 2) for every 4 Poise on self (%d)\n", c->name, gain, c2->Poise[0]);

       sleep(1);

     }

    inflictStatus(c->Poise, 5, 0, 0, 99, 0, 99);
    printf("\n%s gains +5 Poise Stack (%d)\n", c->name, c->Poise[0]);

     sleep(1);

    c->Haste[1] += 3;
    printf("\n%s gains +3 Haste next turn\n", c->name);

     sleep(1);
  }

  // Skill 7
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->skills[6])) {

    if (c->Passive >= 250) {
    c->Passive -= 250;
    if (c->Passive < 0) c->Passive = 0;
    printf("\n%s consumes 250 Mana (%d) to deal +50%% damage\n", c->name, c->Passive);
    ...
    gain = c->Poise[0]/10;
     ...
     if (gain > 0) {
      ...
      printf("\n%s at 10+ Poise Stack (%d), %s gains +1 Coin Power\n", c2->name, c2->Poise[0], c->name);
    ...
    c->Haste[1] += 5;
    printf("\n%s gains +5 Haste next turn\n", c->name);

     sleep(1);
  }

  // Skill 8
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->skills[7])) {

    int consumed = ((c->Passive - 500) > 0) ? 500 : c->Passive ;
    float damage = consumed *0.5f;
    c->Passive -= consumed;
    if (c->Passive < 0) c->Passive = 0;
    printf("\n%s consumes up to 500 Mana (%d) to deal +0.5%% damage (%.1f) for every consumed (%d)\n", c->name, c->Passive, damage, consumed);

    chosenSkill->DamageUp[0] += damage;

     sleep(1);

    printf("\nOn Clash win with this %s's Skill, destroy enemy's all Unbreakable Coins\n", c->name);
     ...
     if (gain > 0) {
      ...
      printf("\n%s gains +1 Base Power (%d - Max 5) for every 4 (Bleed Stack(%d) + Unique Burn Stack(%d)) on target (%d)\n", c->name, gain, c2->Bleed[0], c2->Burn[0], c2->Bleed[0] + c2->Burn[0]);
    ...
    gain = c->Poise[0]/10;
     ...
     if (gain > 0) {
       ...
       chosenSkill->CoinPowerBoost[0] += 5;

      printf("\n%s at 10+ Poise Stack (%d), %s gains +5 Coin Power\n", c2->name, c2->Poise[0], c->name);
    ...
    gain = c2->Tremor[0]/10;
     ...
     if (gain > 0) {
      ...
      printf("\n%s at 10+ Tremor Stack (%d), %s gains +5 Coin Power\n", c2->name, c2->Tremor[0], c->name);

       sleep(1);

     }

    int count = c2->Bleed[0] + c2->Burn[0] + c2->Tremor[0] + c->Poise[0];
    gain = count*3;
    ...
    printf("\n%s deals +3%% damage (%d%% - Max 300%%) for every (Burn Stack on target(%d) + Bleed Stack on target(%d) + Tremor Stack on target(%d) + Poise Stack on self(%d)) (%d)\n", c->name, gain, c2->Burn[0], c2->Bleed[0], c2->Tremor[0], c->Poise[0], count);

    sleep(1);
    }

    inflictStatus(c->Poise, 10, 0, 0, 99, 0, 99);
    printf("\n%s gains +10 Poise Stack (%d)\n", c->name, c->Poise[0]);

     sleep(1);

    c->skills[1].active += 5;
    if (c->skills[1].active > 5) c->skills[1].active = 5;
    printf("\n%s gains +5 Violet Flame Enchant (%d)\n", c->name, c->skills[1].active);

     sleep(1);
  }

  // Skill 9
  if (isId(c->ID, "Shadow") == 0 && (chosenSkill == &c->skills[8])) {
    ...
    inflictStatus(c->Poise, 3, 5, 0, 99, 0, 99);
    printf("\n%s gains +3 Poise Stack (%d) and +5 Poise Count (%d)\n", c->name, c->Poise[0], c->Poise[1]);
  ...
  if (isId(c->ID, "The Middle Little Brother Sinclair") == 0 && chosenSkill == &c->defenseSkill[0]) {
    ...
    if (c->Passive >= 6) {
      ...
      *tempDefense -= chosenSkill->Defense;
      ...
      getEffectiveSkill( c, c2, targetSkill, &c->skills[2], tempOffense, tempDefense);
      ...
      *tempDefense -= chosenSkill->Defense;
      ...
      getEffectiveSkill( c, c2, targetSkill, &c->skills[1], tempOffense, tempDefense);
...
// ---------------------- Meursault:Blade Lineage Mentor ---------------------
 ...
 // ---------------------------- Lei heng -----------------------------
    ...
    c->FinalPowerUp[0] += SkillFinalPowerBoost;
      ...
      float DamageBuff = 0;
      if (c->Speed - c2->Speed >= 3) {
        DamageBuff = (5*((int)(abs(c2->Speed - c->Speed)))) > 20 ? 20 : (5*(int)(abs(c2->Speed - c->Speed)));
        c->DamageUp[0] += DamageBuff;
      }
  ...
  if (isId(c->ID, "Lei heng") == 0 && (chosenSkill == &c->defenseSkill[0]) && (targetSkill != NULL && targetSkill->skillType == 0)) {

    int heal = 10;
    if (c->Sanity < 0) heal += 5;

    updateSanity(c, heal);

    printf("\n%s heals 10 Sanity (%d)\n"
      " - at less than 0 Sanity, heal 5 additionally\n", c->name, c->Sanity);
  ...
  if (isId(c->ID, "Lei heng") == 0 &&
    ...
    if (c->skills[9].active > 0) {

    chosenSkill->ClashPower[0] -= c->skills[9].active * 2; // Lose clash power

    printf("\n%s loses %d Clash Power\n",
      c->name, c->skills[9].active * 2);
...
// ------------------------------ Gregor:Firefist ------------------------------
  ...
  // ------------------------ Dawn Office Fixer Sinclair ----------------------------
    ...
    // Dawn Office Fixer Sinclair - Skill Buff base form
   if (isId(c->ID, "Dawn Office Fixer Sinclair") == 0 && c->Sanity >= 10) {
   ...
   if (isId(c->ID, "Dawn Office Fixer Sinclair") == 0 && c->skills[3].active && c->Sanity >= 20) {
     ...
     if (c->Sanity < 45) {
        ...
        printf("\nWhen in a Volatile E.G.O state, %s at 20+ Sanity, gains 1 Coin Power (%d) for every 20 Sanity (%d Sanity)\n",
               c->name, c->Sanity/20, c->Sanity);

        sleep(1);
     } else {
       c->skills[4].active = 1;
       chosenSkill->CoinPowerBoost[0] += 3;

       printf("\nWhen in a Volatile E.G.O state, %s at 45+ Sanity, gains 3 Coin Power\n",
              c->name);
  ...
  if (isId(c->ID, "Dawn Office Fixer Sinclair") == 0 && c2->Burn[0] >= 4 && (chosenSkill == &c->skills[1])) {

    chosenSkill->ClashPower[0] += 1;

    printf("\n%s at 4+ Burn Stack, %s gains 1 Clash Power\n",
           c2->name, c->name);
  ...
  // Dawn Office Fixer Sinclair - Skill Buff S1
  if (isId(c->ID, "Dawn Office Fixer Sinclair") == 0 && c2->Burn[0] >= 3 && (chosenSkill == &c->skills[0])) {

    int boost = c2->Burn[0]/2;
    if (boost > 4) boost = 4;

    chosenSkill->BasePowerBoost[0] += boost;

    printf("\n%s gains 1 Base Power (%d - Max 4) for every 2 Burn Stack on target (%d)\n",
           c->name, boost, c2->Burn[0]);
  ...
  if (isId(c->ID, "Dawn Office Fixer Sinclair") == 0 && !c->skills[3].active && c2->Burn[0] >= 7 && (chosenSkill == &c->skills[3] || chosenSkill == &c->skills[2])) {

    int boost = c2->Burn[0]/7;
    if (boost > 2) boost = 2;

    chosenSkill->CoinPowerBoost[0] += boost;

    printf("\n%s gains 1 Coin Power (%d - Max 2) for every 7 Burn Stack on target (%d)\n",
           c->name, boost, c2->Burn[0]);
  ...
  if (isId(c->ID, "Dawn Office Fixer Sinclair") == 0 && c->skills[3].active && (chosenSkill == &c->skills[3] || chosenSkill == &c->skills[2])) {
    ...
    if (c2->Burn[0] >= 4) {
    int boost = 2*(c2->Burn[0]/4);
      if (boost > 8) boost = 8;

        chosenSkill->CoinPowerBoost[0] += boost;

    printf("\n%s gains 2 Coin Power (%d - Max 8) for every 4 Burn Stack on target (%d)\n",
           c->name, boost, c2->Burn[0]);
  ...
  // Dawn Office Fixer Sinclair - Skill Buff EGO form Defense
  if (isId(c->ID, "Dawn Office Fixer Sinclair") == 0 && c->skills[3].active && (chosenSkill == &c->defenseSkill[0])) {

    if (c2->Burn[0] >= 2) {
    int boost = 2*(c2->Burn[0]/2);
      if (boost > 20) boost = 20;

        chosenSkill->BasePowerBoost[0] += boost;

    printf("\nWhen in a Volatile E.G.O state, %s gains 2 Base Power (%d - Max 20) for every 2 Burn Stack on target (%d)\n",
           c->name, boost, c2->Burn[0]);
...
void applyClashStartPassives(Character *p1, SkillStats *s1, Character *p2, SkillStats *s2) {
  ...
  // --------------------- Clash Phase ----------------------
    ...
    // Dawn Office Fixer Sinclair - Burn Passion
   if (isId(p1->ID, "Dawn Office Fixer Sinclair") == 0 && p1->defenseSkill[3].active > 0) {

        s1->ClashPower[0] += p1->defenseSkill[2].active;

       p1->defenseSkill[3].active--;

        printf("\n%s gains %d Clash Power from Burn Passion; then lose 1 Count (%d)\n",
          p1->name, p1->defenseSkill[2].active, p1->defenseSkill[3].active);
...
ClashResult ClashableCounter(Character *p1, SkillStats *s1, int playerTempOffense,
  ...
  if (isId(counterUnit->ID, "Don Quixote:The Manager of La Manchaland") ==
    ...
    if (isId(counterUnit->ID, "Don Quixote:The Manager of La Manchaland") ==
      ...
      chosenSkill = getEffectiveSkill(counterUnit, AttackUnit, s1, &counterUnit->defenseSkill[1], &cOff, &cDef);
...
// p1 = winner p2 = loser
void applyClashRoundResult(Character *p1, SkillStats *s1, Character *p2, SkillStats *s2, int winnerCoins, int loserCoins, int clashCount) {
  ...
  // Skill Defense
  if (isId(p1->ID, "Shadow") == 0 && (s1 == &p1->defenseSkill[0]) && loserCoins <= 0) {

    p1->skills[1].active++;
    if (p1->skills[1].active > 5) p1->skills[1].active = 5;
    printf("\n%s won the Clash, gain 1 Violet Flame Enchant (%d)\n", p1->name, p1->skills[1].active = 5);

     sleep(1);
  }

  // Skill 1
  if (isId(p1->ID, "Shadow") == 0 && (s1 == &p1->skills[0]) && loserCoins <= 0) {

    inflictStatus(p1->Poise, 2, 0, 0, 99, 0, 99);
    printf("\n%s won the Clash, gains +2 Poise Stack (%d)\n", p1->name, p1->Poise[0]);

     sleep(1);
  }

  // Skill 2
  if (isId(p1->ID, "Shadow") == 0 && (s1 == &p1->skills[1]) && loserCoins <= 0) {

    inflictStatus(p2->Tremor, 0, 3, 0, 99, 0, 99);
    printf("\n%s won the Clash, inflict +3 Tremor Count (%d)\n", p1->name, p2->Tremor[1]);

     sleep(1);
  }

  // Skill 7
  if (isId(p1->ID, "Shadow") == 0 && (s1 == &p1->skills[6]) && loserCoins <= 0) {

    inflictStatus(p1->Poise, 0, 5, 0, 99, 0, 99);
    printf("\n%s won the Clash, gains +5 Poise Count (%d)\n", p1->name, p1->Poise[1]);

     sleep(1);
  }
  
  // Skill 8
  if (isId(p1->ID, "Shadow") == 0 && (s1 == &p1->skills[7]) && loserCoins <= 0) {

    printf("\n%s won the Clash, destroys enemy's all Unbreakable Coins\n", p1->name);

    sleep(1);

    inflictStatus(p1->Poise, 10, 0, 0, 99, 0, 99);
    printf("\n%s won the Clash, gains +10 Poise Stack (%d)\n", p1->name, p1->Poise[0]);
  ...
  if (isId(p1->ID, "The House of Spiders: The Thumb Nursefather Rodion") == 0 && (s1 == &p1->skills[0]) && p1->skills[11].active == 0 && loserCoins <= 0) {

    int lost = winnerCoins;
  ...
  if (isId(p1->ID, "The House of Spiders: The Thumb Nursefather Rodion") == 0 && (s1 == &p1->skills[2] || s1 == &p1->skills[3]) && p1->skills[11].active == 0 && loserCoins <= 0) {
  ...
  // The One Who Grips Faust Skill 2 won
  if (isId(p1->ID, "The One Who Grips Faust") == 0 &&
      (s1 == &p1->skills[2] || s1 == &p1->skills[3]) && loserCoins <= 0) {
  ...
  if (isId(p2->ID, "Sukuna:King of Curse") == 0 && loserCoins <= 0 && s2 == &p2->skills[7]) {
  ...
  if (isId(p2->ID, "Sukuna:King of Curse") == 0 && loserCoins <= 0 && (s2 == &p2->skills[5] || (s2 == &p2->skills[3] && p2->skills[3].Unbreakable > 0))) {
  ...
  // Jia Qiu S15 lost
  if (isId(p2->ID, "Jia Qiu") == 0 && loserCoins <= 0 && s2 == &p2->skills[15]) {
  ...
  // Heathcliff:Wild Hunt – skill 3 heal sanity
  if (isId(p1->ID, "Heathcliff:Wild Hunt") == 0 &&
      s1 == &p1->defenseSkill[0] && loserCoins <= 0) {
  ...
  if (isId(p2->ID, "Fixer grade 9?") == 0 && loserCoins <= 0) {
    p2->Passive += 1;
  ...
  if (isId(p2->ID, "Fixer grade 9?") == 0 &&
      (s2 == &p2->skills[8] || s2 == &p2->skills[1] || s2 == &p2->skills[2]) && loserCoins <= 0) {
    p2->Passive -= 3;
  ...
  // lose Black Silence
  if (isId(p2->ID, "Fixer grade 9?") == 0 &&
      (s2 == &p2->skills[4]) && loserCoins <= 0 && p2->Passive >= 3) {
    p2->Passive -= 3;
    if (p2->Passive < 0) p2->Passive = 0;
  ...
  if (isId(p2->ID, "Sancho:The Second Kindred of Don Quixote") == 0 &&
      (s2 == &p2->skills[7] || s2 == &p2->skills[8]) && loserCoins <= 0 && p2->Passive >= 4) {
  ...
  if (isId(p2->ID, "Lei heng") == 0 && ((s2 == &p2->skills[0] && p2->skills[0].active >= 1) || (s2 == &p2->skills[1] && p2->skills[0].active >= 1) || s2 == &p2->defenseSkill[0]) &&
    ...
    p2->defenseSkill[1].active -= 1;
      if (p2->defenseSkill[1].active < 0) p2->defenseSkill[1].active = 0;
      ...
      if (p2->defenseSkill[2].active < 0) p2->defenseSkill[2].active = 0;
  ...
  if ((isId(p2->ID, "Lei heng") == 0 && loserCoins <= 0)) {
    int tremorCount = (clashCount / 3);
  ...
  if ((isId(p1->ID, "Lei heng") == 0 && loserCoins <= 0)) {
    int tremorCount = (clashCount / 3);
    if (tremorCount > 3) tremorCount = 3;
    if (tremorCount > 0) {
  ...
  // Lei heng (as enemy/p2) – Prey: player (p1) wins clash → heal 50%% missing HP
  if (isId(p2->ID, "Lei heng") == 0 && p2->skills[6].active == 1 && loserCoins <= 0) {
    int baseheal = (int)((p1->MAX_HP - p1->HP) / 2);
    int heal = baseheal;
    if ((int)p1->HP + heal > (int)p1->MAX_HP) heal = (int)p1->MAX_HP - (int)p1->HP;
      p1->HP += heal;
    p2->skills[9].active++; // Lose clash power at powerful attack
      printf("\n%s won the Clash against %s, heal 50%% missing HP (%d - %.2f/%.2f)\n",
        p1->name, p2->name, baseheal, p1->HP, p1->MAX_HP);
      sleep(1);
  }
  ...
  if (isId(p1->ID, "Dawn Office Fixer Sinclair") == 0 &&
      (s1 == &p1->skills[3] || s1 == &p1->skills[2]) && loserCoins <= 0) {
  ...
  if (isId(p2->ID, "Dawn Office Fixer Sinclair") == 0 && !p2->skills[3].active &&
     (s2 == &p2->skills[3] || s2 == &p2->skills[2]) && loserCoins <= 0) {
  ...
  if (isId(p2->ID, "Erlking Heathcliff") == 0 && loserCoins <= 0) {
  ...
  if (isId(p2->ID, "Jia Qiu") == 0 && s2 == &p2->skills[14] && loserCoins <= 0) {
...
if (isId(p2->ID, "King in Binds") == 0 && (s2 == &p2->skills[1] || s2 == &p2->skills[2] || s2 == &p2->skills[3] || s2 == &p2->skills[4]) && loserCoins <= 0) {
...
// ------------------------------------------------------------------------------------------------------------------------------------
  ...
  if (isId(p1->ID, "The Middle Nursefather - Matthias") == 0 && loserCoins <= 0) {
    ...
    sleep(1);
      ...
      if (p1->skills[5].active == 0 && p1->skills[8].active == 0 && loserCoins <= 0) {
  ...
  // The Middle Nursefather - Matthias - Skill 1
  if (isId(p2->ID, "The Middle Nursefather - Matthias") == 0 && loserCoins <= 0 && (s2 == &p2->skills[0] || s2 == &p2->skills[7])) {
  ...
  // The Middle Nursefather - Matthias - Skill 2
  if (isId(p2->ID, "The Middle Nursefather - Matthias") == 0 && loserCoins <= 0 && s2 == &p2->skills[1]) {
  ...
  if (isId(p2->ID, "The Middle Nursefather - Matthias") == 0 && loserCoins <= 0 && (s2 == &p2->skills[7] || s2 == &p2->skills[8] || s2 == &p2->skills[9])) {
  ...
  if (isId(p1->ID, "The Middle Nursefather - Matthias") == 0 && loserCoins <= 0 && (s1 == &p1->skills[7] || s1 == &p1->skills[8] || s1 == &p1->skills[9])) {
  ...
  if (isId(p1->ID, "Sancho:The Second Kindred of Don Quixote") == 0 &&
    s1 == &p1->skills[6] && loserCoins <= 0 && p1->HP > 1) {
  ...
  if (isId(p1->ID, "Sancho:The Second Kindred of Don Quixote") == 0 &&
    (s1 == &p1->skills[7] || s1 == &p1->skills[8]) && loserCoins <= 0) {
  ...
  if (isId(p1->ID, "Sukuna:King of Curse") == 0 && loserCoins <= 0) {
  ...
  if (isId(p1->ID, "Sukuna:King of Curse") == 0 && loserCoins <= 0 && s1 == &p1->skills[7]) {
  ...
  if (isId(p1->ID, "Lei heng") == 0 && p1->skills[0].active == 2 &&
    ...
    p1->skills[0].active == 3 && loserCoins <= 0) {
  ...
  if (isId(p1->ID, "Erlking Heathcliff") == 0 && loserCoins <= 0) {
  ...
  if (isId(p1->ID, "Fixer grade 9?") == 0 && loserCoins <= 0) {
    p1->Passive += 3;
  ...
  if (isId(p1->ID, "Fixer grade 9?") == 0 && p1->Passive >= 3 && loserCoins <= 0 && s1 == &p1->skills[1]) {
    p1->Passive -= 3;
    s1->DamageUp[0] += 50;
  ...
  if (isId(p1->ID, "Fixer grade 9?") == 0 && p1->Passive >= 5 && loserCoins <= 0 && s1 == &p1->skills[3]) {
    p1->Passive -= 5;
    if (p1->Passive < 0) p1->Passive = 0;
    p2->Paralyze[1] += 2;
  ...
  if (isId(p1->ID, "Fixer grade 9?") == 0 && p1->Passive >= 2 && loserCoins <= 0 && s1 == &p1->skills[5]) {
    p1->Passive -= 3;
    if (p1->Passive < 0) p1->Passive = 0;
    p2->ProtectionDown[0] += 20;
  ...
  if (isId(p1->ID, "Fixer grade 9?") == 0 && p1->Passive >= 2 && loserCoins <= 0 && (s1 == &p1->skills[0] || s1 == &p1->skills[4])) {
    p1->Passive -= 2;
    if (p1->Passive < 0) p1->Passive = 0;
    p1->DamageUp[1] += 15;
  ...
  if (isId(p1->ID, "Fixer grade 9?") == 0 && loserCoins <= 0 && (s1 == &p1->skills[8])) {
    p1->Passive += 3;
    if (p1->Passive > 60) p1->Passive = 60;
    p1->ClashPowerUp[1] += 1;
...
if (isId(p1->ID, "King in Binds") == 0 && (s1 == &p1->skills[0] || s1 == &p1->skills[1] || s1 == &p1->skills[2] || s1 == &p1->skills[4]) && loserCoins <= 0) {
...
ClashResult clashPhase(Character *p1, SkillStats *s1, int playerTempOffense,
  ...
  while (playerCoins > 0 && enemyCoins > 0 && p1->HP > 0 && p2->HP > 0) {
    ...
    if (playerTotal > enemyTotal && enemyCoins > 0) {
      ...
      if (enemyCoins <= 0 && isId(p1->ID, "Shadow") == 0) { 
          // ถ้าฝั่ง Player (Shadow) เป็นผู้ชนะ Clash
          if (s1 == &p1->skills[7]) { // ถ้าใช้ท่า Atomic

              if (enemyUnbreakableLost > 0) {
                      enemyUnbreakableLost = 0; 
              }
          }
      }
      ...
      if (playerCoins <= 0 && isId(p2->ID, "Shadow") == 0) { 
          // ถ้าฝั่ง Enemy (Shadow) เป็นผู้ชนะ Clash
          if (s2 == &p2->skills[7]) { // ถ้าใช้ท่า Atomic

              if (playerUnbreakableLost > 0) {
                    playerUnbreakableLost = 0; 
              }
          }
      }
...
void setupCharacters(Character *player, Character *enemy, int pIndex,
  ...
  player->skills[9] = (SkillStats){
    ...
    player->sanityLossBase = 5;
    player->skills[0] =
        (SkillStats){"Fierce Charge", 5, 7, 1, 2, 0, 1, 1, 0, 3, 1};
    player->skills[1] =
  ...
  else if (pIndex == 109) { // BasePower, CoinPower, Coins, Offense, Defense, DmgMutiplier, active, Unbreakable, Copies, Clashable
    player->name = "Shadow";
    player->HP = 500;
    player->MAX_HP = 500;
    player->MinSpeed = 10;
    ...
    player->skills[0] = (SkillStats){"Speed Blitz", 4, 2, 5, 2, 5, 1, 0, 0, 3, 1};
    player->skills[1] = (SkillStats){"Echo of Shadow's Step", 5, 5, 2, 2, 5, 1, 1, 1, 3, 1};
    player->skills[2] = (SkillStats){"String Master", 5, 4, 3, 4, 5, 1, 1, 1, 2, 1};
    player->skills[3] = (SkillStats){"Slime Suit & Witch's secret art: Control, Expand, Attack", 5, 2, 6, 4, 5, 1, 1, 0, 2, 1};
    player->skills[4] = (SkillStats){"Spark of Dark Flame", 10, 8, 1, 4, 5, 1, 1, 0, 2, 1};
    player->skills[5] = (SkillStats){"Pause Walker", 3, 6, 3, 4, 5, 1, 0, 2, 2, 1};
    player->skills[6] = (SkillStats){"Highest", 6, 3, 5, 6, 5, 1, 0, 5, 1, 1};
    player->skills[7] = (SkillStats){"I Am Atomic...", 20, 30, 1, 10, 5, 1, 0, 0, 0, 1};
    player->skills[8] = (SkillStats){"There is no road to almightiness for those using borrowed strength", 5, 3, 3, 3, 5, 1, 0, 0, 0, 0};

    // 0=Atk, 1=Guard, 2=Evade, 3=Counter, 4=ClashableGuard, 5=ClashableCounter
    player->defenseSkill[0] = (SkillStats){"Overdrive", 10, 5, 1, 0, 5, 1, 1, 1, 0, 1, 4};

    player->Passive = 0; // Mana (0-1000)
    player->skills[0].active = 0;    // True Power (0-10)
    player->skills[1].active = 0;    // Violet Flame Enchant (0-5)
    player->skills[2].active = 0;    // Once per encounter HP flag

    player->numDefenseSkills = 1; // <-- important

    player->numSkills = 9; // <-- important
  }
  ...
  if (eIndex == 0) {
    ...
    enemy->skills[9].active = 0;  // loses Clash Power Count
...
// ------------------------ Shadow ----------------------------
  ...
  // Turn Start
  if (isId(player->ID, "Shadow") == 0) {

    if (TurnCount <= 1 && player->Passive <= 0) {
      player->Passive += 1000;
      printf("\n%s gains 1000 Mana (%d)\n", player->name, player->Passive);
      sleep(1);
    }

      // 2. Playtime is over (True Power Stacking - skills[0].active)
      int tpGain = 1;
      if (player->skills[1].active > 0) tpGain++; // ถ้ามีดาบม่วง (+ skills[1].active) ได้แต้มเพิ่ม
      player->skills[0].active += tpGain;
    if (player->skills[0].active > 10) player->skills[0].active = 10;
    printf("\n%s gains %d True Power (%d)\n", player->name, tpGain, player->skills[0].active);
    sleep(1);

      // 3. HP 50% Trigger (ใช้ flag skills[2].active)
      if ((player->HP <= player->MAX_HP * 0.5 || isStaggered(player)) && player->skills[2].active == 0) {
        ...
        sleep(1);
          ...
          player->skills[2].active = 1; // Mark once per encounter
        if (player->skills[0].active > 10) player->skills[0].active = 10;
      ...
      // 4. True Power Buffs
      int tp = player->skills[0].active;

      // 5. Check if Atomic is ready (True Power 10)
      if (tp >= 10 && *playerSkill1 != 7) {
          *playerSkill1 = 7; // บังคับเปลี่ยนสกิลในมือช่อง 1 เป็น I Am Atomic
        printf("\n%s Replacing Skill with 'I Am Atomic...'\n", player->name);
  ...
  if (isId(player->ID, "Hong lu:The Lord of Hongyuan") == 0) {
    ...
    if (player->Passive >= 1) {
      ...
      player->DamageUp[0] += player->Passive*6;

      printf("\n%s at 1+ Heishou Bolus Contamination [黑獸丸染], gains +1 Max Speed and +6%% Damage Up for every Stack (%d)", player->name, player->Passive);
    ...
    if (player->Passive >= 2) {
      ...
      player->DefenseLevelUp[0] += player->Passive * 2;

      printf("\n%s at 2+ Heishou Bolus Contamination [黑獸丸染], gains +1 Min Speed and +2 Defense Level Up for every Stack (%d)", player->name, player->Passive);
    ...
    if (player->Passive >= 3) {
      ...
      player->AttackSkillPowerUp[0] += 1;

      printf("\n%s at 3+ Heishou Bolus Contamination [黑獸丸染], gains +1 Offense Level Up for every Stack (%d) and gain 1 Attack Skill Power Up", player->name, player->Passive);

     }

    if (player->Passive >= 4) {

      player->OffenseLevelUp[0] += 5;
      player->DamageUp[0] += 20;

      printf("\n%s at 4+ Heishou Bolus Contamination [黑獸丸染], gains +5 more Offense Level Up and +20%% Damage Up (%d)", player->name, player->Passive);
...
// The One Who Grips Faust - Turn Start
  ...
  if (isId(player->ID, "Dawn Office Fixer Sinclair") == 0 && !player->skills[3].active && player->HP <= player->MAX_HP * 0.3 && player->Sanity > -45 && player->skills[0].active) {
    ...
    player->defenseSkill[2].active = clashpowerbuff;
    player->defenseSkill[3].active = 2;


      printf("\n%s exits the Volatile E.G.O::Waxen Pinion state and loses all 'Volatile Passion' to gain +(Volatile Passion x 3) Burn Passion (%d - Max 20)\n", player->name, clashpowerbuff);
  ...
  if (isId(player->ID, "Dawn Office Fixer Sinclair") == 0 && player->skills[3].active) {
    ...
    int damage = 10 * player->Passive;
    if (damage > 50) damage = 50;
    int power = 1 * player->Passive;
    if (power > 5) power = 5;
    int take = 10 * player->Passive;
    if (take > 50) take = 50;

    int burn = player->Passive;

    player->Burn[0] += burn;

    player->FinalPowerUp[0] += power;
      player->DamageUp[0] += 10 * player->Passive;
    player->ProtectionUp[0] += take;

    printf("\n%s gains 1 'Volatile Passion' and gain +%d Burn Stack, gain 1 Final Power (%d - Max 5), gain +10%% Damage (%d%% - Max 50%%) and Take -10%% Damage (%d%% - Max 50%%) for every stack (%d)\n", player->name, burn, power, damage, take, player->Passive);
  ...
  if (isId(player->ID, "Jia Qiu") == 0 && (player->skills[15].active > 0) && enemy->HP <= 0) {
    ...
    enemy->HP = enemy->MAX_HP;
      ...
      printf("\n%s's Uncompromising Imposition activated! Heal up to max HP and gain 1 Final Power Up next turn, lose 1 stack(%d)", enemy->name, player->skills[15].active);
      sleep(1);
  } else {
...
// -------------- Special Status Effects --------------
    ...
    if (player->Tremor[0] > 0 || player->Tremor[1] > 0) {
      ...
      printf("\n%s loses 1 Tremor Count (Tremor - %s | Stack %d Count %d)\n", player->name, player->TremorType, player->Tremor[0], player->Tremor[1]);
    ...
    if (player->Charge[0] > 0 || player->Charge[1] > 0) {
      ...
      printf("\n%s loses 1 Charge Count (Stack %d Count %d)\n", player->name, player->Charge[0], player->Charge[1]);
      }
      ...
      // Burn
        if ((player->Burn[0] > 0 || player->Burn[1] > 0) && player->HP > 0) {
          ...
          if (isId(player->ID, "Dawn Office Fixer Sinclair") == 0 && !player->skills[3].active) damage *= 0.7;
          if (isId(player->ID, "Dawn Office Fixer Sinclair") == 0 && player->skills[3].active) damage *= 0.5;
          if (isId(player->ID, "Dawn Office Fixer Sinclair") == 0 && player->Burn[1] >= 1 && player->Passive > 0) {
              int reductionPercent = player->Passive * 5;
              if (reductionPercent > 40) reductionPercent = 40; // Cap ไว้ที่ 40% ตามเงื่อนไข

              // คำนวณตัวคูณ เช่น ลด 40% คือคูณ 0.6
              float extraMultiplier = (100 - reductionPercent) / 100.0f;
              damage *= extraMultiplier;
          }

          if (isId(enemy->ID, "Shadow") == 0) {
              damage = player->Burn[0] * 2;

              player->Burn[0] += 1; // เพิ่ม Stack
            player->Burn[1]--; // Count Burn min 1
            if (player->Burn[1] <= 0) player->Burn[1] = 0;

            printf("\n%s gains 1 Violet Burn Stack (%d)\n", player->name, player->Burn[0]);
          }

          printf("\n%s takes %d Burn damage (Stack %d Count %d)\n", player->name, damage, player->Burn[0], player->Burn[1]);
 ...
 // The One Who Grips Faust - Nail and Gaze
    ...
    // Turn End
    if (isId(player->ID, "Shadow") == 0) {
        // 1. Mana Regen (Passive)
        player->Passive += 100;
        if (player->Passive > 1000) player->Passive = 1000;
      printf("\n%s gains 100 Mana (%d)\n", player->name, player->Passive);
      sleep(1);

      // 1. Slime Suit Turn End Heal
      int missing = player->MAX_HP - player->HP;
      if (missing > 0 && player->HP > 0) {
          int heal = missing * 0.25;
          player->HP += heal;
          printf("\n%s heals %d HP (%.2f/%.2f)\n", player->name, heal, player->HP, player->MAX_HP);
        sleep(1);
      }

      // 2. ลดแต้ม Violet Flame (skills[1].active) ลง 1 ทุกจบเทิร์น
      if (player->skills[1].active > 0) player->skills[1].active--;
      printf("\n%s loses 1 Violet Flame Enchant (%d)\n", player->name, player->skills[1].active);
      sleep(1);
    ...
    if (isId(player->ID, "The House of Spiders: The Thumb Nursefather Rodion") == 0 && player->HP > 0) {
...
// ------------------------------ The House of Spiders: The Ring Nursefather Hong Lu ------------------------------
    ...
    if (isId(player->ID, "The House of Spiders: The Ring Nursefather Hong Lu") == 0) {
      ...
      printf("\n%s loses 2 Viewing the Tableau (%d)\n", player->name, player->Passive);
        ...
        if (player->Stagger > 0 && player->Passive > 0 && player->skills[3].active == 0 && player->HP > 0) {
            player->Stagger = 0;
            player->skills[3].active = 1; // ใช้ได้ครั้งเดียวต่อ Encounter
            printf("\n%s recovers from Stagger!\n", player->name);
...
if (player->skills[5].active == 1) { // ถ้าทำตาม Prescript สำเร็จ
  ...
  sleep(1);
    ...
    // Reset Speed Skill 3 and 6
    if (isId(enemy->ID, "Lei heng") == 0) {

    if (enemySkillUsed == &enemy->skills[4] && enemy->MinSpeed > 15) { // เช็คว่ามีความเร็วส่วนเกินไหม (ปกติบอสมีแค่ 2-4)
        enemy->MinSpeed -= 15;
        enemy->MaxSpeed -= 15;
    }

      if (enemySkillUsed == &enemy->skills[2] && enemy->MinSpeed > 10) { // เช็คว่ามีความเร็วส่วนเกินไหม (ปกติบอสมีแค่ 2-4)
          enemy->MinSpeed -= 10;
          enemy->MaxSpeed -= 10;
      }
    ...
    if (isId(enemy->ID, "Jia Qiu") == 0 && enemy->HP <= enemy->MAX_HP * 0.6 &&
      ...
      if (isId(player->ID, "Hong lu:The Lord of Hongyuan") == 0) {
        ...
        sleep(1);
         ...
         printf("\n%s gains 3 Uncompromising Imposition (Turn End heal 5 Sanity, When HP drop to 0 heal up to max HP and gain 1 Final Power next turn; then lose 1 stack)\n", player->name);
      } else {
        sleep(1);
  ...
  printf("\n%s snapping Bandages, cap HP to 20%% for this turn, apply 'Bandages of the King in Binds' on all enemies.\n", player->name);
    ...
    if (player->Sanity <= -45 && player->SanityFreezeTurns != 1) {
      ...
      if (player->SanityFreezeTurns == 0) {
        ...
        else if (isId(player->ID, "Shadow") == 0) {

          player->Sanity = 30;

            printf("\n%s reset Sanity to 30 instead\n", player->name);
        }
      } 
    }
...
printf("\n 14. Niðbrandr\n Gain by using Certain Skill:\n"
  ...
  "\n\x1b[1;30m A Black Sword relic, wield to power deep than the ground and the hell, opposite with the one White Sword relic, how ironically that something that this vast opposite can be wield by one person\x1b[0m\n");
    ...
    if (isId(enemy->ID, "Lei heng") == 0 && enemy->HP <= enemy->MAX_HP*0.2) {
      ...
      if (isId(enemy->ID, "Jia Qiu") == 0 && enemy->HP <= 0) {
        ...
        int *enemyLastUnused)
{
  ...
  while (player->HP > 0 && enemy.HP > 0) {
    ...
    if (IsplayerUnableToAct) {
      ...
      getEffectiveSkill(player, &enemy, selectedEnemyPtr, &dummy, &playerTempOffense, &playerTempDefense);
      playerSkillEffective = NULL;
    } else {
      playerSkillEffective = getEffectiveSkill(player, &enemy, selectedEnemyPtr, playerSkillEffective,
                        &playerTempOffense, &playerTempDefense);
    }

    if (IsenemyUnableToAct) {
      SkillStats dummyK = enemy.defenseSkill[0];
      getEffectiveSkill(&enemy, player, playerSkillEffective, &dummyK, &enemyTempOffense, &enemyTempDefense);
      enemySkillEffective = NULL;
    } else {
      enemySkillEffective = getEffectiveSkill(&enemy, player, playerSkillEffective, selectedEnemyPtr,
    ...
    while (player->HP > 0 && boss->HP > 0) {
      ...
      *enemyLastUnused, boss->numSkills);
      ...
      if (IsplayerUnableToAct) {
          ...
          getEffectiveSkill(player, boss, enemySkillEffective, &dummy, &playerTempOffense, &playerTempDefense);
          playerSkillEffective = NULL; // บังคับเป็น NULL ตามที่ต้องการ
      } else {
          playerSkillEffective = getEffectiveSkill(player, boss, enemySkillEffective, playerSkillEffective,
                                                  &playerTempOffense, &playerTempDefense);
      }
      ...
      if (IsenemyUnableToAct) {
          ...
          getEffectiveSkill(boss, player, playerSkillEffective, &dummyE, &enemyTempOffense, &enemyTempDefense);
          enemySkillEffective = NULL; // บังคับเป็น NULL
      } else {
          enemySkillEffective = getEffectiveSkill(boss, player, playerSkillEffective, enemySkillEffective,
                                                  &enemyTempOffense, &enemyTempDefense);
      }
...
int main() {
  ...
  while (1) {
    ...
    if (scanf("%d", &selected_identity) == 1) {
      ...
      if (selected_identity == 0) {
        ...
        if (scanf("%49s", secret_code) != 1) {
          ...
          if (strcasecmp(secret_code, "Muga") == 0) {
            ...
            else if (strcasecmp(secret_code, "Shadow") == 0 || strcasecmp(secret_code, "Cid") == 0) {
                printf("\n\x1b[1;35m[!!!]: My Name is Shadow. He who lurks in shadow in order to hunt shadow, now come ate me\x1b[0m\n");
                sleep(1);
                selected_identity = 110; // Internal flag for Shadow
            }
          else {
      ...
      if (selected_identity == 100) targetIndex = 99;      // Muga
        ...
        else if (selected_identity == 110) targetIndex = 110; // Shadow
    else targetIndex = -1;
        ...
        if (selected_identity - 1 == 0) {
         ...
         printf("\n 5. Tigermark Round\n 'Unique Ammo', Skill Coins that spend Tigermark Round gain +1 Power and deal +10%% damage (activates only as long as the Coin has Rounds left to spend)\n" 
            " - At 1+ Tigermark Round and (Tigermark Rounds gained/4)+ Tigermark Round spent, convert all Coins of 'Tanglecleaver' into Unbreakable Coins\n");
        ...
        printf("\n 7. Overheat\n Min & Max Speed +2, Attack Skills Lose (cumulative number of Tigermark Rounds & Savage Tigermark Rounds spent / (cumulative number of Tigermark Rounds & Savage Tigermark Rounds Gained/5)) Clash Power (Max 5); however, gain the following effects(cumulative):\n"
         ...
         " - (Sum of Savage Tigermark Round + (Tigermark Rounds/2) Gained)+ Rounds spent: On Clash Lose, Unbreakable Coins of this unit's Attack Skills deal +(75 + missing HP percentage on self)%% damage (Max 150%%)\n"
          ...
          printf("\n 6. Shadow-vested Bladesinger [着影揮刀]\n If this unit using 'To Claim Their Bones' immediately after activated 'Swordplay of the Homeland', 'To Claim Their Bones' gains following effect:\n"
            ...
            "  - On Hit with final coin deal +((Poise Stack on self + Poise Count on self))%% damage\n"
          ...
          //Story / Lore
          printf("Born into an unimaginably wealthy, ancient noble family, this carefree young master views the grim realities of the world as nothing more than an entertaining playground. However, he was born solely to serve as an 'eye' for an Elder who sought to escape an eternal, boring life by witnessing joy through his left eye. Forced to endure torture just to provide amusement for the Elder, Hong Lu's unbearable will led him to gouge out his own eyeball. Even though it is now wrapped in bandages, they are perpetually soaked in a red liquid—perhaps a sign that the Elder still lingers. Despite this, he does not care. Backed by elite tutors and a mystical bloodline, he leads an army known as the Heishou Pack, a group of augmented fighters serving the upper classes of Hongyuan. His innate combat talent is nothing short of terrifying. Moving across the battlefield with effortless, aristocratic grace, he wields a Guandao—a flowing glaive—carving through armor with a cheerful smile. He slew all his enemies, including his little sister who was beyond saving, and eventually slaughtered the family that tortured him. Now, he stands alone as the 'Lord of Hongyuan.'.\n\n");

          //Description
          printf("A low HP character with great buffs, strong damage output, an anti-death passive, and followers that can help you. However, for maximum output, you need to use Skill 3 multiple times\n\n");
          ...
          printf("\n 5. Hardblood\n When use certain skills gain randomly 2-4 Hardblood. When hit by enemy gain 2 Hardblood (Max 30; 'Hardblood' cannot drop below 1)"
            ...
            //Story / Lore
            printf("An invisible bell resounds within the containment unit, where this calm Safety Team Agent engages in mutual dialogue with the Abnormality known as the Funeral of the Dead Butterflies. While others find their minds crumbling, Yi Sang's just heart and patience allow him to suppress the creature's impulses through a gentle, silent understanding. But as the First Warning Trumpet activates and the Ordeal of Violet descends, the time for respite ends. Standing back-to-back with his Captain, he wields dual handguns imbued with butterflies, firing upon the invading creatures. He carries out his duty in solemn silence, turning his muzzles toward the lost foes with a quiet remark: 'Fruits of Understanding... have you not lost your ways? I mourn you... bloom, and be the path for one another to follow.'\n\n");
            ...
            printf("\n 3. Reload\n When runs out of 'The Living & The Departed', or this unit equipped Defense Skill, Turn End uses 'Reload', while attacking, stop attack and use 'Reload' instead, when 'Reload' is used, spend (30 - (The Living & The Departed))/2 Sanity to gain 20 'The Living & The Departed' and gain Shield HP equal to (Butterfly on the target x 2)%% of Max HP. (Max 40%%)\n");
             printf("\n 4. FromTheCoffinAButterflyTakesFlight\n On Clash Win: Enemy loses Sanity based on Skills used (Enemy with loses Sanity on Clash Win, gains Sanity instead; enemy without Sanity gains 'Butterfly' instead)\n");
              ...
              //Passive
              printf("Passive Skills:\n");
              printf("\n 1. Unstable Shell of Ego\n Turn Start: At 40+ Sanity, consume 20 Sanity to enter the Volatile E.G.O::Waxen Pinion state\n"
                " - At 30%% or less HP and if this unit's Sanity isn't at -45 at Turn End, reset Sanity to 45; then, enter the Volatile E.G.O::Waxen Pinion state. (Once per Encounter) (this 'Turn Start' effect does not activate repeatedly)\n"
                
                "\n This unit takes 30%% less damage from Burn\n"
                " - When at Volatile E.G.O::Waxen Pinion state, take 50%% less damage from Burn instead\n"
                " · At 1+ Burn Count, take 5%% less damage from Burn for every Volatile Passion Stack (Max 40%%)");
              printf("\n 2. Determination\n Turn Start: At 0 or less SP, if in the Volatile E.G.O::Waxen Pinion state; exit the Volatile E.G.O::Waxen Pinion state and loses all 'Volatile Passion' to gain +(Volatile Passion x 3) Burn Passion Stack\n"
                
                "\n Burn Passion\n"
                " - Max Stack: 20\n"
                " - Base Count: 2\n"
                " - Gain Clash Power equal to Stack\n"
                " - On Clash: Lose 1 Count\n"
                " - When reach 0 Count, expire\n");
               printf("\n 3. Volatile Passion\n "
                 " - On hit, inflict 1 Burn Stack\n"
                 " - If this unit equipped Defense Skill, inflict 1 Burn Stack against attacker per hit\n"
                 " - Turn Start: Gain 1 'Volatile Passion' and Gain +(Volatile Passion Stack) Burn Stack while in the Volatile E.G.O state, gain 1 Final Power (Max 5), gain +10%% Damage (Max 50%%) and Take -10%% Damage (Max 50%%) for every stack\n"
                 " - Turn End: lose 5 Sanity for every stack (Max 40 Sanity)\n");
              printf("\n 4. Stigma Workshop Weaponry / Passion\n At sum of 30+ Burn Stack inflicted in this Encounter, gain following effect:\n"
                " - This unit inflicts +1 more Burn Stack and Count with its Skills\n"
              ...
              printf("\n 5. Burn\n Infliction by Certain Skills:\n"
                " - Max Stack: 99\n"
                " - Max Count: 99\n"
               " - At 1+ Count, or at 1+ Stack (Turn End: When triggering, at 1+ Count and 0 Stack, Count as 1 Stack, if at 0 Count and 1+ Stack, Count as 1 Count)\n"
               " - Turn End: Take fixed damage equal to (Stack). Then lose 1 Count.\n"
                   ...
                   printf(" Gain the following effect next turn based on Stack:"
                    ...
                    printf("\n 10. The Index Nursefather\n Upon entering the Encounter for the first time, gain 'Wound-casing Mask'"
                     ...
                     printf(" \n13. Oracle Device [Caduceus]\n A random weapon is assigned to every Coin for Base Attack Skills. Each weapon has a unique effect."
                      ...
                      "\n - When caving in the back of the skull with a hammer... (Skill 1 deal +15%% damage for this Coin, This Coin deal +5%% damage, On Hit without Cracking: Trigger 'Tremor Burst' with 3 Tremor Stacks, if target took 20 damage from 'Tremor Burst' in this Encounter, if this unit not on 'Stagger' state, enter 'Stagger' state (Cannot act for one turn and take +50%% damage) and reset this progess)"
        ...
        printf("\n 4. An Arbiter\n Gains +50%% damage, Final Power +2, Deal +20%% damage and +10 Base Power for every Fairy on enemy, when getting attack by Full Cracking Coin, Take -80%% damage and gain (50 + Missing HP/3) Shield HP (Max 100 per attacked), when getting attack and at 0+ Sanity, consumes 10 Sanity to gain (100 + Missing HP/2) Shield HP (Max 300 per attacked)\n");
            } else if (targetIndex == 110) {
            //Taunt
            printf("\"Run? who's running? where? WHYYYY!'\n\n");

                   //Story / Lore
                   printf("Cid Kagenou (name as Shadow) is a reincarnated teen who treats a very real, world-controlling cult conspiracy as a giant chuunibyou roleplay game with his fanatically loyal subordinates, accidentally becoming the ultimate all-powerful mastermind he always dreamed of being.\n\n");

            //Description
            printf("Even if I master every martial art on Earth, I can never defeat a nuclear weapon. A human body would be instantly vaporized. I realized that to become an Eminence in Shadow, I needed to become something that could not be destroyed by a nuke.\n\n");

            //Passive
               printf("Passive Skills:\n");
                     printf("\n 1. Violet Burn\n When Inflicted by Certain Skills: At 1+ Count, or at 1+ Stack (Turn End: When triggering, at 1+ Count and 0 Stack, Count as 1 Stack, if at 0 Count and 1+ Stack, Count as 1 Count)\n"
                       " - Unique Burn"
                       " - Max Stack: 99\n"
                       " - Max Count: 99\n"
                       " - Turn End: Gain +1 Stack; then take fixed Burn Damage equal to (Stack x 2) after that lose 2 Count\n"
                       " - When reach 0 Count, loses all Stack too\n");
               printf("\n 2. Tremor\n When Inflicted by Certain Skills: At 1+ Count, or at 1+ Stack (When triggering, at 1+ Count and 0 Stack, gain 1 Stack, if at 0 Count and 1+ Stack, gain 1 Count), When Trigger by 'Tremor Burst', Raise Stagger Threshold equal to Stack; then reduce 1 Count, if this unit's Stagger Threshold at 100 in this Encounter, if this unit not on 'Stagger' state, enter 'Stagger' state (Cannot act for one turn and take +50%% damage) and reset this progess. Turn End: Lose 1 Count. When reach 0 Count, loses all Stack too (Max 99 Stack/Count)\n");
               printf("\n 3. Poise\n When Gain by Certain Skills: At 1+ Count, or at 1+ Stack (When triggering, at 1+ Count and 0 Stack, Count as 1 Stack, if at 0 Count and 1+ Stack, Count as 1 Count), When Attack: Have a (Stack x 5)%% chance to 'Critical Hit' (Boost damage by 20%%). If this unit deals Critical Hit, reduce 1 Count. When reach 0 Count, loses all Stack too (Max 99 Stack/Count)\n");
                     printf("\n 4. Mana\n Encounter Start: Gain 1000 \x1b[0;33mMana\x1b[0m\n"
                       " - Can be Consumed by certain Skills\n"
                       " - Turn End: Gain 100 \x1b[0;33mMana\x1b[0m\n"
                       " - Max Stack: 1000\n");
                     printf("\n 5. Silme Suit\n"
                       " - Take -10%% Damage for every 100 \x1b[0;33mMana\x1b[0m on self (Max 50%%)\n"
                       " - Turn End: Heal 25%% missing HP\n"
               "\n\x1b[1;30m The Suit that creation by him, independent and flexible but still strong using Mana to control and shape them\x1b[0m\n");
                     printf("\n 6. Silme Weapon\n"
                       " - On Uncreaking final Coin, deal (20%% of final Coin's damage) Damage as bonus damage\n"
                       " - On Hit, deal (5%% of Coin's damage) Damage as bonus damage\n"
                       " - On Hit, inflict +1 \x1b[0;31mBleed\x1b[0m Stack\n"
                       "  · If this unit has \x1b[0;33mViolet Flame Enchant\x1b[0m, inflict +1 Violet Burn Stack and Count as well\n"

                       " \n\x1b[0;33mViolet Flame Enchant\x1b[0m\n"
                       " - Max Stack: 5\n"
                       " - Gain by certain Skills\n"
                       " - Deal +20%% Damage\n"
                       " - If target has \x1b[0;31mViolet Burn\x1b[0m, deal +50%% Damage instead\n"
                       " - Turn End: Lose 1 Stack\n"
                       
               "\n\x1b[1;30m The Weapon that creation by him, independent and flexible but still strong using Mana to control and shape them\x1b[0m\n");
                     printf("\n 7. I'll Show you\n At 5+ Haste or 15+ Speed:\n"
                       " - Combat Start:\n"
                       "  · Gain 2 \x1b[0;33mViolet Flame Enchant\x1b[0m\n"
                       "  · Use \"There is no road to almightiness for those using borrowed strength\" as Unopposed attack (Once per turn)\n"

                       "\n\x1b[1;30m There is no road to almightiness for those who use borrowed strength. None!\x1b[0m\n");
                       printf("\n 8. Playtime is over\n"
                         ...
                         " - At less than 50%% or less HP, or Staggered, Gain 10 \x1b[0;33mTrue Power\x1b[0m (Once per Encounter)\n"
                         ...
                         " - Max Stack: 10\n"
                         " - Gain 1 Offense Level Up and 1 Defense Level Up for every Stack\n"
                         " - Combat Start: This unit heals Sanity equal to Sanity\n"
                         " - Can be consumed by Certain Skills\n"

                       "\n\x1b[1;30m Once upon a time there was a man who wished to withstand a nuclear bomb. The man developed his muscles, honed his mind, perfected his skills. However, there were yet heights he could not attain. But I couldn't allow myself to give up. So after years upon years of training, I arrived at a single solution. If I don't want to be vaporized in a nuclear explosion... I simply have to become nuclear myself!\x1b[0m\n");
                     printf("\n 9. Panic Recovery\n Turn End: if in Panic, reset SP to 30.\n");
                     printf("\n 10. Fixed Panic\n This unit's Panic Type does not change when inflicted with an effect that changes Panic Types. Instead, this unit is inflicted with an effect that is inflicted against Non-SP Units.\n");
                     printf("\n 11. Panic Type - Shadō Gāden\n"
                       " Low Morale:\n"
  ...
  } else if (selected_identity == 110) {
    printf("You selected Shadow\n");
  } else {
        ...
        if (scanf("%d", &selected_enemy) == 1) {
          ...
          if (selected_enemy - 1 == 0) {
            ...
            printf("\n 5. Tiantui Star's Blade [天退星刀]\n Unopposed Attacks deal +30%% damage On Hit and inflict +10%% Take Damage Up next turn\n"
             ...
             "\n Skill Coins that spent Tigermark Round deal +20%% damage On Hit and inflict 1 Defense Level Down next turn\n"
            ...
            printf(" - Heal Sanity equal to Speed value. At less than 0 Sanity, double the heal amount");
            ...
            printf("\n 12. Chosen Prey\n at 90%% or less HP, or at the end of the 3nd turn\n"
              ...
              "\n - If this unit wins a Clash against Lei Heng while under this effect, heal 50%% missing HP"
              "\n - Expires at Turn End; this unit heals 10 Sanity\n"

              "\nFor every times Lei Heng loses the Clash against the target with Prey\n"
                " - The powerful attacks 'Tanglecleaver' and 'Savage Tigerslayer's Perfected Flurry of Blades' loses 2 Clash Power\n"
                " - On used the powerful attacks reset this progress\n"
            ...
            printf("\n 13. The Thumb's Officer, Capo IIII\n In a Clash, inflict +(# of Clashes / 3) Tremor Count (Max 3)\n");
            printf("\n 14. Burn\n When Inflicted by Certain Skills: At 1+ Count, or at 1+ Stack (Turn End: When triggering, at 1+ Count and 0 Stack, Count as 1 Stack, if at 0 Count and 1+ Stack, Count as 1 Count), Turn End: Take fixed damage equal to (Stack). Then lose 1 Count. When reach 0 Count, loses all Stack too (Max 99 Stack/Count)\n");
            printf("\n 15. Tremor\n When Inflicted by Certain Skills: At 1+ Count, or at 1+ Stack (When triggering, at 1+ Count and 0 Stack, gain 1 Stack, if at 0 Count and 1+ Stack, gain 1 Count), When Trigger by 'Tremor Burst', Raise Stagger Threshold equal to Stack; then reduce 1 Count, if this unit's Stagger Threshold at (target's Max HP) in this Encounter, if this unit not on 'Stagger' state, enter 'Stagger' state (Cannot act for one turn and take +50%% damage) and reset this progess. Turn End: Lose 1 Count. When reach 0 Count, loses all Stack too (Max 99 Stack/Count)\n");
            printf("\n 16. Tremor - Scorch\n Change when Triggered by Amplitude Conversion\n"
               " - On Tremor Burst, Take (Tremor Stack and Burn Stack / 2) fixed Damage and Raise Stagger Threshold equal to Stack; then reduce 1 Count, if this unit's Stagger Threshold at (target's Max HP) in this Encounter, if this unit not on 'Stagger' state, enter 'Stagger' state (Cannot act for one turn and take +50%% damage) and reset this progess.\n"
               " - Turn End: Lose 1 Count. When reach 0 Count, loses all Stack and Change back to Normal 'Tremor' too (Max 99 Stack/Count)\n");
          ...
          printf("\n 9. Tremor\n When Inflicted: At 1+ Count, or at 1+ Stack (When triggering, at 1+ Count and 0 Stack, Count as 1 Stack, if at 0 Count and 1+ Stack, Count as 1 Count), When Trigger by 'Tremor Burst', Raise Stagger Threshold equal to Stack; then reduce 1 Count, if this unit's Stagger Threshold at (Max HP/4) in this Encounter, if this unit not on 'Stagger' state, enter 'Stagger' state (Cannot act for one turn and take +50%% damage) and reset this progess. Turn End: Lose 1 Count. When reach 0 Count, loses all Stack too (Max 99 Stack/Count)\n");
  ...
  if (selected_enemy - 1 == 6) {
    ...
    if (!IsplayerUnableToAct) {
      ...
      if (IsplayerUnableToAct) {
          SkillStats dummy = player.defenseSkill[0]; // โคลนค่าจากท่าป้องกัน 0
          getEffectiveSkill(&player, &enemy, enemySkillEffective, &dummy, &playerTempOffense, &playerTempDefense);
          playerSkillEffective = NULL; // บังคับเป็น NULL ตามที่ต้องการ
      } else {
          playerSkillEffective = getEffectiveSkill(&player, &enemy, enemySkillEffective, playerSkillEffective,
                                                  &playerTempOffense, &playerTempDefense);
      }
      ...
      if (IsenemyUnableToAct) {
          ...
          getEffectiveSkill(&enemy, &player, playerSkillEffective, &dummyE, &enemyTempOffense, &enemyTempDefense);
          enemySkillEffective = NULL; // บังคับเป็น NULL
      } else {
          enemySkillEffective = getEffectiveSkill(&enemy, &player, playerSkillEffective, enemySkillEffective,
                                                  &enemyTempOffense, &enemyTempDefense);
      }
